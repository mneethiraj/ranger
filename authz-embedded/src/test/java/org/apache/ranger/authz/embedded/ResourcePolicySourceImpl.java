...
public class ResourcePolicySourceImpl extends AbstractRangerAdminClient {
    private static final Logger LOG = LoggerFactory.getLogger(ResourcePolicySourceImpl.class);
    ...
    public void init(String serviceName, String appId, String configPropertyPrefix, Configuration config) {
        ...
        String directory  = config.get(configPropertyPrefix + ".policy.source.resource.path");
        String pathPrefix = directory + File.separator + appId + "_" + serviceName;
    ...
    public ServicePolicies getServicePoliciesIfUpdated(long lastKnownVersion, long lastActivationTimeInMillis) {
        ...
        if (lastKnownVersion == -1 || policies == null || policies.getPolicyVersion() == null || !policies.getPolicyVersion().equals(lastKnownVersion)) {
            return policies;
        }
    ...
    public RangerRoles getRolesIfUpdated(long lastKnownVersion, long lastActivationTimeInMills) {
        ...
        if (lastKnownVersion == -1 || roles == null || roles.getRoleVersion() == null || !roles.getRoleVersion().equals(lastKnownVersion)) {
            return roles;
        }
    ...
    public ServiceTags getServiceTagsIfUpdated(long lastKnownVersion, long lastActivationTimeInMillis) {
        ...
        if (lastKnownVersion == -1 || tags == null || tags.getTagVersion() == null || !tags.getTagVersion().equals(lastKnownVersion)) {
            return tags;
        }
    ...
    public RangerUserStore getUserStoreIfUpdated(long lastKnownVersion, long lastActivationTimeInMillis) {
        ...
        if (lastKnownVersion == -1 || userStore == null || userStore.getUserStoreVersion() == null || !userStore.getUserStoreVersion().equals(lastKnownVersion)) {
            return userStore;
        }
    ...
    public ServiceGdsInfo getGdsInfoIfUpdated(long lastKnownVersion, long lastActivationTimeInMillis) {
        ...
        if (lastKnownVersion == -1 || gdsInfo == null || gdsInfo.getGdsVersion() == null || !gdsInfo.getGdsVersion().equals(lastKnownVersion)) {
            return gdsInfo;
        }
